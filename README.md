...
- Регистрация по логину и паролю (без подтверждения почты).
- Логин и получение JWT-токена (`access_token`).
- Работа только со своими сущностями (pages, tags).
- Страницы:
  - иерархия через `parent_id`;
  - markdown-контент;
